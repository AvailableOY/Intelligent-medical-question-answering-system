...
from utils.Optimize_RAG_Performance_Subproblems import rag_subproblems
...
def ai_chat_123(request):
    # 获取是否要进行联网搜索的Boolean
    is_online = request.POST.get("isOnline", " ")
    is_online = True if is_online == "1" else False
    ...
    if topic_id == 0:
        ...
        if topic_obj and topic_obj.summary:
        # 只添加一条“系统记忆”或“主题背景”，不加历史对话
            messages.append({
                "role": "system",
                "content": f"当前对话主题摘要：{topic_obj.summary}"
            })
            # # 查询该 topic 下的所有聊天记录（按时间排序）
            # chat_records = Chat.objects.filter(topic_id=topic_id).order_by('create_time')
    ...
    # 先对问题进行多条分解
    question_list = []
    question_list = rag_subproblems(question)
    ...
    # web_search_triggered = is_online  # 标记是否触发了联网搜索
    # if len(context_lists) == 0:
    #     web_search_triggered = True
    #     print("⚠️ 本地知识库未命中，尝试联网搜索...")
    # print("知识库内容：",context_text,"web_search_triggered",web_search_triggered)
    # web_search_triggered = False  # 标记是否触发了联网搜索

    if is_online:
        print("用户强制使用联网搜索")
        web_results = perform_web_search(question)  
        context_lists.extend(web_results)
        context_text = "\n\n".join(context_lists)
